...
async def retrieve_user_resources_for_workspace_service(
    ...
    if ("WorkspaceResearcher" in user.roles or "AirlockManager" in user.roles) and ("WorkspaceOwner" not in user.roles or "ImperialWorkspaceResearcher" not in user.roles or "ImperialWorkspaceOwner" not in user.roles or "ImperialWorkspaceManager" not in user.roles):
