...
def validate_user_has_valid_role_for_user_resource(user, user_resource):
    ...
    if "ImperialAirlockManager" in user.roles:
        return
                
    if ("WorkspaceResearcher" in user.roles or "AirlockManager" in user.roles or "ImperialWorkspaceResearcher" in user.roles or "ImperialWorkspaceOwner" in user.roles or "ImperialWorkspaceDataEngineer" in user.roles or "ImperialAirlockManager" in user.roles) and user_resource.ownerId == user.id:
        return
