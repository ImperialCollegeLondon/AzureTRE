...
workspace_templates_admin_router = APIRouter(dependencies=[Depends(get_current_imperial_admin_user)])
...
async def get_workspace_templates(authorized_only: bool = False, template_repo=Depends(get_repository(ResourceTemplateRepository)), user=Depends(get_current_imperial_admin_user)) -> ResourceTemplateInformationInList:
