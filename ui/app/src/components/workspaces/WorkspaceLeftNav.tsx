...
export const WorkspaceLeftNav: React.FunctionComponent<WorkspaceLeftNavProps> = (props: WorkspaceLeftNavProps) => {
  ...
  useEffect(() => {
    const getWorkspaceServices = async () => {
      ...
      if (workspaceCtx.workspace.properties !== undefined && workspaceCtx.workspace.properties.enable_airlock && (workspaceCtx.roles.includes(WorkspaceRoleName.AirlockManager) ||  workspaceCtx.roles.includes(WorkspaceRoleName.ImperialWorkspaceDataEngineer) ||  workspaceCtx.roles.includes(WorkspaceRoleName.ImperialAirlockManager))) {
