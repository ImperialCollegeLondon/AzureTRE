  ...
  useEffect(() => {
    const getWorkspaceCosts = async () => {
      try {
        ...
        if(wsRoles.includes(WorkspaceRoleName.WorkspaceOwner) || wsRoles.includes(WorkspaceRoleName.ImperialWorkspaceOwner) || wsRoles.includes(WorkspaceRoleName.ImperialWorkspaceManager)){
          let scopeId = (await apiCall(`${ApiEndpoint.Workspaces}/${workspaceId}/scopeid`, HttpMethod.Get)).workspaceAuth.scopeId;
